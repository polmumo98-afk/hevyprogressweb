...
// Clasificación de patrón de movimiento para el balance muscular.
const MOVEMENT = {
  chest: "push",
  shoulders: "push",
  triceps: "push",
...
function movementOf(key) {
  return MOVEMENT[key] || null;
}
...
export function buildDashboard(workouts, templates) {
  ...
  const movementSets = { push: 0, pull: 0, legs: 0, core: 0 };
  ...
  for (const w of sorted) {
    ...
    for (const ex of w.exercises || []) {
      ...
      const movement = movementOf(muscleKey);
      ...
      for (const set of ex.sets || []) {
        ...
        if (movement) movementSets[movement] += 1;
  ...
  // --- Balance muscular (patrones de movimiento) ---
  const totalMovementSets =
    movementSets.push + movementSets.pull + movementSets.legs + movementSets.core;
  const upper = movementSets.push + movementSets.pull;
  const lower = movementSets.legs;

  const balanceNotes = [];
  if (movementSets.push > 0 && movementSets.pull > 0) {
    const ratio = movementSets.push / movementSets.pull;
    if (ratio > 1.35) {
      balanceNotes.push(
        "Haces bastante más empuje que tirón. Añade algo de espalda para equilibrar hombro."
      );
    } else if (ratio < 0.74) {
      balanceNotes.push(
        "Haces más tirón que empuje. Puedes sumar algo de pecho/hombro."
      );
    } else {
      balanceNotes.push("Empuje y tirón bien equilibrados. 👌");
    }
  }
  if (upper > 0 && lower > 0) {
    const ratioUL = upper / lower;
    if (ratioUL > 2.2) {
      balanceNotes.push(
        "Tren superior muy por encima del inferior. No te olvides de pierna."
      );
    } else if (ratioUL < 0.7) {
      balanceNotes.push("Mucho tren inferior; equilibra con tren superior.");
    }
  }
  if (totalMovementSets > 0 && movementSets.core / totalMovementSets < 0.05) {
    balanceNotes.push("Poco core. Un par de series de abdominales no vienen mal.");
  }

  const pct = (n) =>
    totalMovementSets ? Math.round((n / totalMovementSets) * 100) : 0;

  const muscleBalance = {
    push: movementSets.push,
    pull: movementSets.pull,
    legs: movementSets.legs,
    core: movementSets.core,
    pushPct: pct(movementSets.push),
    pullPct: pct(movementSets.pull),
    legsPct: pct(movementSets.legs),
    corePct: pct(movementSets.core),
    pushPullRatio:
      movementSets.pull > 0
        ? Math.round((movementSets.push / movementSets.pull) * 100) / 100
        : null,
    upperLowerRatio:
      lower > 0 ? Math.round((upper / lower) * 100) / 100 : null,
    notes: balanceNotes,
  };
  ...
  // --- Vista de esta semana (lunes a domingo) ---
  const dayFull = ["Lunes", "Martes", "Miércoles", "Jueves", "Viernes", "Sábado", "Domingo"];
  ...
  for (let i = 0; i < 7; i++) {
    ...
    d.setDate(d.getDate() + i);
    const key = iso(d);
    const dayWorkouts = allWorkoutRows
      .filter((w) => w.date === key)
      .map((w) => ({
        title: w.title,
        volume: w.volume,
    ...
    weekView.push({
      ...
      volume: dayWorkouts.reduce((a, b) => a + b.volume, 0),
      sets: dayWorkouts.reduce((a, b) => a + b.sets, 0),
    });
  }
  ...
  return {
    ...
    recentWorkouts: allWorkoutRows.slice(-10).reverse(),
  };
}
