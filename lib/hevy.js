...
export async function fetchAllRoutines() {
  ...
  const pageSize = 10;
  for (let i = 0; i < 100; i++) {
    const data = await hevyFetch("/routines", { page, pageSize });
...
export async function fetchAllRoutineFolders() {
  const folders = [];
  let page = 1;
  const pageSize = 10;
  for (let i = 0; i < 100; i++) {
    const data = await hevyFetch("/routine_folders", { page, pageSize });
    const batch = data.routine_folders || [];
    folders.push(...batch);
    const pageCount = data.page_count || 1;
    if (page >= pageCount || batch.length === 0) break;
    page += 1;
